...
Given(`Navigate to page with URL {string} and scroll to Expanding Gallery module`, (URL) => {
    ...
    offSCroll()
    scrollDownToBottom()
    cy.get('.mod-expanding-gallery').should('be.visible').scrollIntoView({ offset: { top: -100 }}).wait(2000)
})

Given(`I want to hover icon {string}`, (icon) => {
    cy.get(`.mod-expanding-gallery .swiper-button-${icon}`).realHover({scrollBehavior : false}).wait(500)
})
...
Given(`I want to click icon {string}`, (icon) => {
    ...
    cy.get(`.mod-expanding-gallery .swiper-button-${icon}`).realClick({scrollBehavior : false}).wait(500)
})
    ...
    .each(($span, index, $spans) => {
        if (index >= 1 && index < $spans.length - 1) {
            ...
            if (!isActive) {
                ...
                cy.wrap($span).should('be.visible').realClick({scrollBehavior : false}).wait(500).then(() => {
